    ...
    constructor({ common, stateManager }) {
        ...
        this._common =
            ...
            new common_1.Common({
                ...
                hardfork: common_1.Hardfork.Petersburg
            })
    ...
    async touchAccount(address) {
        ...
        if (!bindedToShard) {
            if (this.isSandboxExecution) bindedToShard = global.KLY_EVM_OPTIONS.bindContext
                ...
                global.ATOMIC_BATCH.put('SHARD_BIND:' + addressAsStringWithout0x, {
                    shard: this.evmContext
                })
        ...
        let evmExecutionContext = this.isSandboxExecution
            ? global.KLY_EVM_OPTIONS.bindContext
            : this.evmContext

        if (
            bindedToShard !== evmExecutionContext &&
            addressAsStringWithout0x !== global.KLY_EVM_OPTIONS.coinbase
        ) {
            throw new Error(
                `Account 0x${addressAsStringWithout0x} binded to shard ${bindedToShard}, but you try to change the state of it via tx on shard ${evmExecutionContext}`
            )
        }
