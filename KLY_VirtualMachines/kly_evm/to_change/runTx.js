...
async function runTx(opts) {
    ...
    if (
        ...
        opts.block.header.gasLimit < opts.tx.gasLimit
    ) {
    ...
    if (
        opts.tx.supports(tx_1.Capability.EIP2718TypedTransaction) &&
        ...
        if (
            opts.tx.supports(tx_1.Capability.EIP1559FeeMarket) &&
...
async function _runTx(opts) {
    ...
    if (
        ...
        !fromAccount.codeHash.equals(util_1.KECCAK256_NULL)
    ) {
        const msg = _errorMsg(
    ...
    if (tx instanceof tx_1.BlobEIP4844Transaction) {
        ...
        const castTx = tx
        totalDataGas =
            castTx.common.param('gasConfig', 'dataGasPerBlob') * BigInt(castTx.numBlobs())
    ...
    if (this.DEBUG) {
        debug(
            `Running tx=0x${tx.isSigned() ? tx.hash().toString('hex') : 'unsigned'} with caller=${caller} gasLimit=${gasLimit} to=${to?.toString() ?? 'none'} value=${value} data=0x${(0, util_1.short)(data)}`
        )
    }
    ...
    if (this.DEBUG) {
        ...
        debug('-'.repeat(100))
        debug(
            `Received tx execResult: [ executionGasUsed=${executionGasUsed} exceptionError=${exceptionError ? `'${exceptionError.error}'` : 'none'} returnValue=0x${(0, util_1.short)(returnValue)} gasRefund=${results.gasRefund ?? 0} ]`
        )
    }
    /*
     * Parse results
    ...
    if (this.DEBUG) {
        debug(
            `tx run finished hash=${opts.tx.isSigned() ? opts.tx.hash().toString('hex') : 'unsigned'} sender=${caller}`
        )
    }
    return results
}
...
async function generateTxReceipt(tx, txResult, cumulativeGasUsed) {
    ...
    if (this.DEBUG) {
        debug(
            `Generate tx receipt transactionType=${tx.type} cumulativeBlockGasUsed=${cumulativeGasUsed} bitvector=${(0, util_1.short)(baseReceipt.bitvector)} (${baseReceipt.bitvector.length} bytes) logs=${baseReceipt.logs.length}`
        )
    }
