...
import { checkDeepEqual, getKeyImage, getRandomBigNumber, hashPoint, toHexString } from './utils'
...
export function sign(
    ...
    if (
        check_u.x.toString(10) != check_r.x.toString(10) ||
...
export function verify(sig /*: RingSign*/, keyList: string[] = []): boolean {
    ...
    keyList.forEach(pk => {
        ...
        digestedExpectedCensus.push(
            keyPair.getPublic().getX().toString(10).concat(keyPair.getPublic().getY().toString(10))
        )
    })
    ring.forEach(kp => {
        digestedRingCensus.push(
            kp.getPublic().getX().toString(10).concat(kp.getPublic().getY().toString(10))
        )
    })
...
export function link(sig_a: any, sig_b: any): boolean {
    return (
        sig_a.I.x.toString(10) == sig_b.I.x.toString(10) &&
        sig_a.I.y.toString(10) == sig_b.I.y.toString(10)
    )
}
