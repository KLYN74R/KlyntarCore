import {
    GET_FROM_APPROVEMENT_THREAD_STATE,
    USE_TEMPORARY_DB
} from '../common_functions/approvement_thread_related.js'
...
let TIME_IS_OUT_FOR_CURRENT_SHARD_LEADER = (
    epochHandler,
    indexOfCurrentLeaderInSequence,
    leaderShipTimeframe
) => {
    ...
    return (
        GET_UTC_TIMESTAMP() >=
        epochHandler.startTimestamp + (indexOfCurrentLeaderInSequence + 2) * leaderShipTimeframe
    )
}
...
export let SET_LEADERS_SEQUENCE_FOR_SHARDS = async (epochHandler, epochSeed) => {
    ...
    for (let reservePoolPubKey of epochHandler.poolsRegistry.reservePools) {
        ...
        let poolStorage = await GET_FROM_APPROVEMENT_THREAD_STATE(
            reservePoolPubKey + `(POOL)_STORAGE_POOL`
        )

        if (poolStorage) {
            let { reserveFor } = poolStorage

            if (!reservePoolsRelatedToShard.has(reserveFor))
                reservePoolsRelatedToShard.set(reserveFor, [])
    ...
    */
    ...
    for (let primePoolID of primePoolsPubKeys) {
        let arrayOfReservePoolsRelatedToThisShard =
            reservePoolsRelatedToShard.get(primePoolID) || []
...
export let SHARDS_LEADERS_MONITORING = async () => {
    ...
    for (let primePoolPubKey of epochHandler.poolsRegistry.primePools) {
        ...
        let leaderSequenceHandler = currentEpochMetadata.SHARDS_LEADERS_HANDLERS.get(
            primePoolPubKey
        ) || {
            currentLeader: -1
        }
        ...
        if (leaderSequenceHandler.currentLeader !== -1) {
            ...
            pubKeyOfCurrentShardLeader =
                epochHandler.leadersSequence[primePoolPubKey][indexOfCurrentLeaderInSequence]
        } else {
        ...
        let itsNotFinishOfSequence =
            epochHandler.leadersSequence[primePoolPubKey][indexOfCurrentLeaderInSequence + 1]

        if (
            itsNotFinishOfSequence &&
            TIME_IS_OUT_FOR_CURRENT_SHARD_LEADER(
                epochHandler,
                indexOfCurrentLeaderInSequence,
                WORKING_THREADS.APPROVEMENT_THREAD.WORKFLOW_OPTIONS.LEADERSHIP_TIMEFRAME
            )
        ) {
            // Inform websocket server that we shouldn't generate proofs for this leader anymore
            currentEpochMetadata.SYNCHRONIZER.set(
                'STOP_PROOFS_GENERATION:' + pubKeyOfCurrentShardLeader,
                true
            )
            ...
            if (
                !currentEpochMetadata.SYNCHRONIZER.has(
                    'GENERATE_FINALIZATION_PROOFS:' + pubKeyOfCurrentShardLeader
                )
            ) {
                ...
                await USE_TEMPORARY_DB(
                    'put',
                    currentEpochMetadata.DATABASE,
                    'LEADERS_HANDLER:' + primePoolPubKey,
                    newLeadersHandler
                )
                    .then(() => {
                        ...
                        currentEpochMetadata.SHARDS_LEADERS_HANDLERS.delete(
                            pubKeyOfCurrentShardLeader
                        )
                        ...
                        currentEpochMetadata.SHARDS_LEADERS_HANDLERS.set(
                            epochHandler.leadersSequence[primePoolPubKey][
                                newLeadersHandler.currentLeader
                            ],
                            primePoolPubKey
                        )
