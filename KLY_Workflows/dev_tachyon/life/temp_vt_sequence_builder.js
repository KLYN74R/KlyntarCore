...
export let BUILD_TEMPORARY_SEQUENCE_OF_VERIFICATION_THREAD = async () => {
    ...
    */
    ...
    for (let primePoolPubKey of vtEpochHandler.poolsRegistry.primePools) {
        localVersionOfCurrentLeaders[primePoolPubKey] =
    ...
    let response = await fetch(
        randomTarget.url + '/data_to_build_temp_data_for_verification_thread',
        optionsToSend
    )
        .then(r => r.json())
        .catch(() => ({}))
    ...
    */
    ...
    for (let [primePoolPubKey, metadata] of Object.entries(response)) {
        if (typeof primePoolPubKey === 'string' && typeof metadata === 'object') {
            ...
            if (
                typeof proposedIndexOfLeader === 'number' &&
                typeof firstBlockByCurrentLeader === 'object' &&
                typeof afpForSecondBlockByCurrentLeader === 'object'
            ) {
                if (
                    localVersionOfCurrentLeaders[primePoolPubKey] <= proposedIndexOfLeader &&
                    firstBlockByCurrentLeader.index === 0
                ) {
                    // Verify the AFP for second block(with index 1 in epoch) to make sure that block 0(first block in epoch) was 100% accepted

                    let afpIsOk = await VERIFY_AGGREGATED_FINALIZATION_PROOF(
                        afpForSecondBlockByCurrentLeader,
                        vtEpochHandler
                    )

                    afpIsOk &&=
                        afpForSecondBlockByCurrentLeader.prevBlockHash ===
                        Block.genHash(firstBlockByCurrentLeader)
                    ...
                    if (afpIsOk) {
                        ...
                        let {
                            isOK,
                            filteredReassignments: filteredReassignmentsInBlockOfProposedLeader
                        } = await CHECK_ALRP_CHAIN_VALIDITY(
                            primePoolPubKey,
                            firstBlockByCurrentLeader,
                            vtLeadersSequences[primePoolPubKey],
                            proposedIndexOfLeader,
                            vtEpochFullID,
                            vtEpochHandler,
                            true
                        ...
                        if (isOK) {
                            let collectionOfAlrpsFromAllThePreviousLeaders = [
                                filteredReassignmentsInBlockOfProposedLeader
                            ] // each element here is object like {pool:{index,hash,firstBlockHash}}
                            ...
                            */
                            ...
                            if (position >= localVersionOfCurrentLeaders[primePoolPubKey]) {
                                ...
                                while (true) {
                                    for (
                                        ;
                                        position >= localVersionOfCurrentLeaders[primePoolPubKey];
                                        position--
                                    ) {
                                        let poolOnThisPosition =
                                            position === -1
                                                ? primePoolPubKey
                                                : vtLeadersSequences[primePoolPubKey][position]

                                        let alrpForThisPoolFromCurrentSet =
                                            currentAlrpSet[poolOnThisPosition]

                                        if (alrpForThisPoolFromCurrentSet.index !== -1) {
                                            // Ask the first block and extract next set of ALRPs

                                            let firstBlockInThisEpochByPool = await GET_BLOCK(
                                                vtEpochHandler.id,
                                                poolOnThisPosition,
                                                0
                                            )
                                            ...
                                            if (
                                                ...
                                                Block.genHash(firstBlockInThisEpochByPool) ===
                                                    alrpForThisPoolFromCurrentSet.firstBlockHash
                                            ) {
                                                let alrpChainValidation =
                                                    position === -1
                                                        ? { isOK: true, filteredReassignments: {} }
                                                        : await CHECK_ALRP_CHAIN_VALIDITY(
                                                              primePoolPubKey,
                                                              firstBlockInThisEpochByPool,
                                                              vtLeadersSequences[primePoolPubKey],
                                                ...
                                                if (alrpChainValidation.isOK) {
                                                    ...
                                                    collectionOfAlrpsFromAllThePreviousLeaders.push(
                                                        alrpChainValidation.filteredReassignments
                                                    )
                                ...
                                if (shouldChangeThisShard) {
                                    ...
                                    let tempReassignmentChain =
                                        tempReassignmentOnVerificationThread[vtEpochFullID][
                                            primePoolPubKey
                                        ].reassignments // poolPubKey => {index,hash}
                                    ...
                                    for (let reassignStats of collectionOfAlrpsFromAllThePreviousLeaders.reverse()) {
                                        ...
                                        for (let [poolPubKey, descriptor] of Object.entries(
                                            reassignStats
                                        )) {
                                            if (!tempReassignmentChain[poolPubKey])
                                                tempReassignmentChain[poolPubKey] = descriptor
                                        }
                                    }
                                    ...
                                    tempReassignmentOnVerificationThread[vtEpochFullID][
                                        primePoolPubKey
                                    ].currentLeader = proposedIndexOfLeader
                                }
                            }
                        }
                    }
    ...
    setTimeout(
        ...
        CONFIGURATION.NODE_LEVEL.TEMPORARY_REASSIGNMENTS_BUILDER_TIMEOUT
    )
}
