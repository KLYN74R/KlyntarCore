import { EPOCH_STILL_FRESH, IS_MY_VERSION_OLD } from '../utils.js'
...
let DELETE_POOLS_WITH_LACK_OF_STAKING_POWER = async (validatorPubKey, fullCopyOfQuorumThread) => {
    ...
    let poolStorage = await GET_FROM_APPROVEMENT_THREAD_STATE(
        validatorPubKey + '(POOL)_STORAGE_POOL'
    )
    ...
    let arrayToDeleteFrom =
        fullCopyOfQuorumThread.EPOCH.poolsRegistry[
            poolStorage.isReserve ? 'reservePools' : 'primePools'
        ]
...
let EXECUTE_EPOCH_EDGE_OPERATIONS = async (
    atomicBatch,
    ...
    for (let operation of epochEdgeOperations) {
        if (operation.type === 'SLASH_UNSTAKE')
            await EPOCH_EDGE_OPERATIONS_VERIFIERS.SLASH_UNSTAKE(operation.payload, false, true)
    }
    ...
    for (let operation of epochEdgeOperations) {
        ...
        */
        await EPOCH_EDGE_OPERATIONS_VERIFIERS[operation.type](
            operation.payload,
            false,
            true,
            fullCopyOfQuorumThread
    ...
    let toRemovePools = [],
        promises = [],
        allThePools = epochHandlerReference.poolsRegistry.primePools.concat(
            epochHandlerReference.poolsRegistry.reservePools
        )

    for (let poolPubKey of allThePools) {
        let promise = GET_FROM_APPROVEMENT_THREAD_STATE(poolPubKey + '(POOL)_STORAGE_POOL').then(
            poolStorage => {
                if (
                    poolStorage.totalPower < fullCopyOfQuorumThread.WORKFLOW_OPTIONS.VALIDATOR_STAKE
                )
                    toRemovePools.push(poolPubKey)
            }
        )
    ...
    for (let address of toRemovePools) {
        deletePoolsPromises.push(
            DELETE_POOLS_WITH_LACK_OF_STAKING_POWER(address, fullCopyOfQuorumThread)
        )
    }
    ...
    for (let poolIdentifier of slashObjectKeys) {
        ...
        // Remove from pools
        let arrayToDeleteFrom =
            fullCopyOfQuorumThread.EPOCH.poolsRegistry.reservePools[
                slashObject[poolIdentifier].isReserve ? 'reservePools' : 'primePools'
            ]
...
export let FIND_AGGREGATED_EPOCH_FINALIZATION_PROOFS = async () => {
    ...
    */
    ...
    if (!EPOCH_STILL_FRESH(WORKING_THREADS.APPROVEMENT_THREAD)) {
        ...
        let epochCache =
            (await BLOCKCHAIN_DATABASES.EPOCH_DATA.get(`EPOCH_CACHE:${oldEpochFullID}`).catch(
                () => null
            )) || {} // {shardID:{firstBlockCreator,firstBlockHash,aefp,firstBlockOnShardFound}}
        ...
        for (let [primePoolPubKey] of entries) {
            ...
            if (!epochCache[primePoolPubKey])
                epochCache[primePoolPubKey] = { firstBlockOnShardFound: false }

            if (
                epochCache[primePoolPubKey].aefp &&
                epochCache[primePoolPubKey].firstBlockOnShardFound
            ) {
            ...
            */
            ...
            if (!epochCache[primePoolPubKey].aefp) {
                ...
                let aefp = await BLOCKCHAIN_DATABASES.EPOCH_DATA.get(
                    `AEFP:${qtEpochHandler.id}:${primePoolPubKey}`
                ).catch(() => false)
                ...
                if (aefp) {
                    ...
                    for (let peerURL of allKnownPeers) {
                        let itsProbablyAggregatedEpochFinalizationProof = await fetch(
                            peerURL +
                                `/aggregated_epoch_finalization_proof/${qtEpochHandler.id}/${primePoolPubKey}`
                        )
                            .then(r => r.json())
                            .catch(() => false)

                        if (itsProbablyAggregatedEpochFinalizationProof) {
                            let aefpPureObject = await VERIFY_AGGREGATED_EPOCH_FINALIZATION_PROOF(
                                itsProbablyAggregatedEpochFinalizationProof,
                                qtEpochHandler.quorum,
                                majority,
                                oldEpochFullID
                            )
        ...
        if (totalNumberOfShards === totalNumberOfReadyShards) {
            ...
            for (let [primePoolPubKey] of entries) {
                ...
                let firstBlockOnThisShard = await GET_BLOCK(
                    qtEpochHandler.id,
                ...
                if (
                    firstBlockOnThisShard &&
                    Block.genHash(firstBlockOnThisShard) ===
            ...
            if (!cycleWasBreak) {
                ...
                let fullCopyOfQuorumThread = JSON.parse(
                    JSON.stringify(WORKING_THREADS.APPROVEMENT_THREAD)
                )
                ...
                await EXECUTE_EPOCH_EDGE_OPERATIONS(
                    atomicBatch,
                    fullCopyOfQuorumThread,
                    epochEdgeOperations
                )
                ...
                await BLOCKCHAIN_DATABASES.EPOCH_DATA.put(
                    `NEXT_EPOCH_HASH:${oldEpochFullID}`,
                    nextEpochHash
                ).catch(() => false)
                ...
                await BLOCKCHAIN_DATABASES.EPOCH_DATA.put(
                    `NEXT_EPOCH_LS:${oldEpochFullID}`,
                    fullCopyOfQuorumThread.EPOCH.leadersSequence
                ).catch(() => false)

                LOG(
                    `\u001b[38;5;154mEpoch edge operations were executed for epoch \u001b[38;5;93m${oldEpochFullID} (QT)\u001b[0m`,
                    COLORS.GREEN
                )

                //_______________________ Update the values for new epoch _______________________

                fullCopyOfQuorumThread.EPOCH.startTimestamp =
                    qtEpochHandler.startTimestamp +
                    fullCopyOfQuorumThread.WORKFLOW_OPTIONS.EPOCH_TIME

                fullCopyOfQuorumThread.EPOCH.id = nextEpochId

                fullCopyOfQuorumThread.EPOCH.hash = nextEpochHash

                fullCopyOfQuorumThread.EPOCH.quorum = GET_CURRENT_EPOCH_QUORUM(
                    fullCopyOfQuorumThread.EPOCH.poolsRegistry,
                    fullCopyOfQuorumThread.WORKFLOW_OPTIONS,
                    nextEpochHash
                )

                await BLOCKCHAIN_DATABASES.EPOCH_DATA.put(
                    `NEXT_EPOCH_QUORUM:${oldEpochFullID}`,
                    fullCopyOfQuorumThread.EPOCH.quorum
                ).catch(() => false)

                // Create new temporary db for the next epoch
                let nextTempDB = level(process.env.CHAINDATA_PATH + `/${nextEpochFullID}`, {
                    valueEncoding: 'json'
                })
                ...
                LOG(
                    `Epoch on quorum thread was updated => \x1b[34;1m${nextEpochHash}#${nextEpochId}`,
                    COLORS.GREEN
                )
                ...
                if (IS_MY_VERSION_OLD('APPROVEMENT_THREAD')) {
                    LOG(
                        `New version detected on APPROVEMENT_THREAD. Please, upgrade your node software`,
                        COLORS.YELLOW
                    )

                    console.log('\n')
                    console.log(
                        fs.readFileSync(PATH_RESOLVE('images/events/update.txt')).toString()
                    )
                ...
                fs.rm(
                    process.env.CHAINDATA_PATH + `/${oldEpochFullID}`,
                    { recursive: true },
                    () => {}
                )
                ...
                let iAmInTheQuorum = WORKING_THREADS.APPROVEMENT_THREAD.EPOCH.quorum.includes(
                ...
                if (EPOCH_STILL_FRESH(WORKING_THREADS.APPROVEMENT_THREAD) && iAmInTheQuorum) {
                    ...
                    WORKING_THREADS.APPROVEMENT_THREAD.EPOCH.poolsRegistry.primePools.forEach(
                        poolPubKey =>
                            currentEpochManager.set(poolPubKey, {
                                index: -1,
                    ...
                    WORKING_THREADS.APPROVEMENT_THREAD.EPOCH.poolsRegistry.reservePools.forEach(
                        poolPubKey =>
                            currentEpochManager.set(poolPubKey, {
                                index: -1,
                                hash: '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef',
