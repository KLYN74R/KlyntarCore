import {
    GET_PSEUDO_RANDOM_SUBSET_FROM_QUORUM_BY_TICKET_ID,
    GET_QUORUM_MAJORITY
                ...
                client.on('connect', connection => {
                    connection.on('message', async message => {
                        if (message.type === 'utf8') {
                            ...
                            if (
                                parsedData.finalizationProof &&
                                proofsGrabber.huntingForHash === parsedData.votedForHash &&
                                FINALIZATION_PROOFS.has(proofsGrabber.huntingForBlockID)
                            ) {
                                if (parsedData.type === 'tmb') {
                                    let dataThatShouldBeSigned =
                                        proofsGrabber.acceptedHash +
                                        proofsGrabber.huntingForBlockID +
                                        proofsGrabber.huntingForHash +
                                        epochFullID

                                    let finalizationProofIsOk =
                                        FINALIZATION_PROOFS.has(proofsGrabber.huntingForBlockID) &&
                                        epochHandler.quorum.includes(parsedData.voter) &&
                                        (await ED25519_VERIFY(
                                    ...
                                    if (
                                        finalizationProofIsOk &&
                                        FINALIZATION_PROOFS.has(proofsGrabber.huntingForBlockID)
                                    ) {
                                        FINALIZATION_PROOFS.get(
                                            proofsGrabber.huntingForBlockID
                                        ).set(parsedData.voter, parsedData.finalizationProof)
                                    }
                                    ...
                                    let tmbProofIsOk = await ED25519_VERIFY(
                                        dataThatShouldBeSigned,
                                        parsedData.tmbProof,
                                        parsedData.voter
                                    )
                                    ...
                                    if (
                                        ...
                                        FINALIZATION_PROOFS.get(
                                            'TMB:' + proofsGrabber.huntingForBlockID
                                        ).set(parsedData.voter, parsedData.tmbProof)
                                    }
                                }
                            }
                        }
...
let RUN_FINALIZATION_PROOFS_GRABBING = async (epochHandler, proofsGrabber) => {
    ...
    let blockIDForHunting =
        epochHandler.id +
        ':' +
        CONFIGURATION.NODE_LEVEL.PUBLIC_KEY +
        ':' +
        (proofsGrabber.acceptedIndex + 1)
    ...
    let blockToSend =
        TEMP_CACHE.get(blockIDForHunting) ||
        (await BLOCKCHAIN_DATABASES.BLOCKS.get(blockIDForHunting).catch(() => null))
    ...
    if (finalizationProofsMapping.size < majority) {
        ...
        if (tmbProofsMapping.size >= 21) {
            ...
            let subsetToSendBlocks = GET_PSEUDO_RANDOM_SUBSET_FROM_QUORUM_BY_TICKET_ID(
                0,
                epochHandler
            )
    ...
    if (finalizationProofsMapping.size >= majority) {
        ...
        */
        ...
        await BLOCKCHAIN_DATABASES.EPOCH_DATA.put(
            'AFP:' + blockIDForHunting,
            aggregatedFinalizationProof
        ).catch(() => false)

        LOG(
            `Approved height for epoch \u001b[38;5;50m${epochHandler.id} \x1b[31;1mis \u001b[38;5;50m${proofsGrabber.acceptedIndex} \x1b[32;1m(${(finalizationProofsMapping.size / epochHandler.quorum.length).toFixed(3) * 100}% agreements)`,
            COLORS.RED
        )
        ...
        await USE_TEMPORARY_DB('put', DATABASE, 'PROOFS_GRABBER', proofsGrabber)
            .then(() => {
