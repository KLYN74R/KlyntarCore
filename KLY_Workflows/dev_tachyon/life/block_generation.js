import { GET_ALL_KNOWN_PEERS } from '../utils.js'
...
let GET_TRANSACTIONS = () =>
    NODE_METADATA.MEMPOOL.splice(
        0,
        WORKING_THREADS.APPROVEMENT_THREAD.WORKFLOW_OPTIONS.TXS_LIMIT_PER_BLOCK
    )

let GET_EPOCH_EDGE_OPERATIONS = epochFullID => {
    if (!EPOCH_METADATA_MAPPING.has(epochFullID)) return []

    let epochEdgeOperationsMempool =
        EPOCH_METADATA_MAPPING.get(epochFullID).EPOCH_EDGE_OPERATIONS_MEMPOOL

    return epochEdgeOperationsMempool.splice(
        0,
        WORKING_THREADS.APPROVEMENT_THREAD.WORKFLOW_OPTIONS.EPOCH_EDGE_OPERATIONS_LIMIT_PER_BLOCK
    )
}
...
let GET_AGGREGATED_EPOCH_FINALIZATION_PROOF_FOR_PREVIOUS_EPOCH = async () => {
        ...
        let aefpProof = await BLOCKCHAIN_DATABASES.EPOCH_DATA.get(
            `AEFP:${WORKING_THREADS.GENERATION_THREAD.epochIndex}:${shardID}`
        ).catch(() => null)
        ...
        if (aefpProof) return aefpProof
            ...
            for (let nodeEndpoint of allKnownNodes) {
                ...
                let itsProbablyAggregatedEpochFinalizationProof = await fetch(finalURL)
                    .then(r => r.json())
                    .catch(() => false)

                let aefpProof =
                    itsProbablyAggregatedEpochFinalizationProof?.shard === shardID &&
    ...
    GET_AGGREGATED_LEADER_ROTATION_PROOF = async (
        epochHandler,
        pubKeyOfOneOfPreviousLeader,
        ...
        let afpForFirstBlock = await GET_VERIFIED_AGGREGATED_FINALIZATION_PROOF_BY_BLOCK_ID(
            firstBlockIDByThisLeader,
            epochHandler
        )

        let firstBlockHash

        let localFinalizationStatsForThisPool = currentEpochMetadata.FINALIZATION_STATS.get(
            pubKeyOfOneOfPreviousLeader
        ) || {
            index: -1,
        ...
        if (localFinalizationStatsForThisPool.index === -1) {
            localFinalizationStatsForThisPool.hash =
        ...
        if (!afpForFirstBlock && localFinalizationStatsForThisPool.index === -1)
            firstBlockHash = '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'
        ...
        if (firstBlockHash) {
            ...
            for (let descriptor of quorumMembers) {
                let responsePromise = fetch(descriptor.url + '/leader_rotation_proof', sendOptions)
                    .then(r => r.json())
                    .then(response => {
        ...
        */
            ...
            for (let result of results) {
                if (result.type === 'OK' && typeof result.sig === 'string') {
                    let signatureIsOk = await ED25519_VERIFY(
                        dataThatShouldBeSigned,
                        result.sig,
                        result.pubKey
                    )
                    ...
                    let blockIdInAfp =
                        epochHandler.id + ':' + pubKeyOfOneOfPreviousLeader + ':' + index

                    if (
                        typeof afp === 'object' &&
                        hash === afp.blockHash &&
                        blockIdInAfp === afp.blockID &&
                        (await VERIFY_AGGREGATED_FINALIZATION_PROOF(afp, epochHandler))
                    ) {
                        ...
                        if (localFinalizationStatsForThisPool.index < index) {
                            ...
                            localFinalizationStatsForThisPool.afp = {
                                prevBlockHash,
                                blockID,
                                blockHash,
                                proofs
                            ...
                            currentEpochMetadata.FINALIZATION_STATS.set(
                                pubKeyOfOneOfPreviousLeader,
                                localFinalizationStatsForThisPool
                            )
...
let GENERATE_BLOCKS_PORTION = async () => {
    ...
    if (
        proofsGrabber &&
        WORKING_THREADS.GENERATION_THREAD.nextIndex > proofsGrabber.acceptedIndex + 1
    )
        return
    ...
    if (!currentEpochMetadata.TEMP_CACHE.has('CAN_PRODUCE_BLOCKS')) {
        let poolPresent = epochHandler.poolsRegistry[
            CONFIGURATION.NODE_LEVEL.PRIME_POOL_PUBKEY ? 'reservePools' : 'primePools'
        ].includes(CONFIGURATION.NODE_LEVEL.PUBLIC_KEY)
    ...
    let myDataInShardsLeadersMonitoring = currentEpochMetadata.SHARDS_LEADERS_HANDLERS.get(
    ...
    if (WORKING_THREADS.GENERATION_THREAD.epochFullId !== epochFullID) {
        ...
        if (epochIndex !== 0) {
            let aefpForPreviousEpoch =
                await GET_AGGREGATED_EPOCH_FINALIZATION_PROOF_FOR_PREVIOUS_EPOCH()
        ...
        WORKING_THREADS.GENERATION_THREAD.prevHash =
            '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'
    ...
    if (typeof myDataInShardsLeadersMonitoring === 'string') {
        ...
        if (WORKING_THREADS.GENERATION_THREAD.nextIndex === 0) {
            ...
            let myIndexInLeadersSequenceForShard = leadersSequenceOfMyShard.indexOf(
                CONFIGURATION.NODE_LEVEL.PUBLIC_KEY
            )

            // Get all previous pools - from zero to <my_position>

            let pubKeysOfAllThePreviousPools = leadersSequenceOfMyShard
                .slice(0, myIndexInLeadersSequenceForShard)
                .reverse()
            ...
            */
            ...
            for (let pubKeyOfPreviousLeader of pubKeysOfAllThePreviousPools) {
                let aggregatedLeaderRotationProof = await GET_AGGREGATED_LEADER_ROTATION_PROOF(
                    epochHandler,
                    pubKeyOfPreviousLeader,
                    indexOfPreviousLeaderInSequence,
                    myPrimePool
                ).catch(() => null)

                if (aggregatedLeaderRotationProof) {
                    extraData.aggregatedLeadersRotationProofs[pubKeyOfPreviousLeader] =
    ...
    */
    ...
    let numberOfBlocksToGenerate = Math.ceil(
        NODE_METADATA.MEMPOOL.length /
            WORKING_THREADS.APPROVEMENT_THREAD.WORKFLOW_OPTIONS.TXS_LIMIT_PER_BLOCK
    )
    ...
    extraData.epochEdgeOperations = GET_EPOCH_EDGE_OPERATIONS(
        WORKING_THREADS.GENERATION_THREAD.epochFullId
    )
    ...
    for (let i = 0; i < numberOfBlocksToGenerate; i++) {
        ...
        BLOCKLOG(
            ...
            hash,
            blockCandidate,
            WORKING_THREADS.GENERATION_THREAD.epochIndex
        )
