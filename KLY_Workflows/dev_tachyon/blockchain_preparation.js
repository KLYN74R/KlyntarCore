...
import { DECRYPT_KEYS, IS_MY_VERSION_OLD } from './utils.js'
...
let resolveDatabase = name =>
    level(process.env.CHAINDATA_PATH + `/${name}`, { valueEncoding: 'json' })
...
let RESTORE_METADATA_CACHE = async () => {
    ...
    for (let poolPubKey of allThePools) {
        ...
        let { index, hash, afp } = (await currentEpochMetadata.DATABASE.get(poolPubKey).catch(
            () => null
        )) || {
            index: -1,
        ...
        if (poolsRegistry.primePools.includes(poolPubKey)) {
            let leadersHandler = await currentEpochMetadata.DATABASE.get(
                'LEADERS_HANDLER:' + poolPubKey
            ).catch(() => false) // {currentLeader:<pointer to current reserve pool in (QT/VT).EPOCH.leadersSequence[<primePool>]>}
            ...
            if (leadersHandler) {
                ...
                let currentLeaderPubKey =
                    WORKING_THREADS.APPROVEMENT_THREAD.EPOCH.leadersSequence[poolPubKey][
                        leadersHandler.currentLeader
                    ]
    ...
    let itsTimeForTheNextEpoch = await currentEpochMetadata.DATABASE.get('TIME_TO_NEW_EPOCH').catch(
        () => false
    )
...
export let SET_GENESIS_TO_STATE = async () => {
        ...
        if (!isReserve) {
            ...
            if (evmStateForThisShard) {
                ...
                for (let evmKey of evmKeys) {
                    ...
                    let caseIgnoreAccountAddress = Buffer.from(evmKey.slice(2), 'hex').toString(
                        'hex'
                    )
    ...
    Object.keys(BLOCKCHAIN_GENESIS.STATE).forEach(addressOrContractID => {
        if (BLOCKCHAIN_GENESIS.STATE[addressOrContractID].type === 'contract') {
            let { lang, balance, uno, storages, bytecode, shard } =
                BLOCKCHAIN_GENESIS.STATE[addressOrContractID]
            ...
            for (let storageID of BLOCKCHAIN_GENESIS.STATE[addressOrContractID].storages) {
                ...
                atomicBatch.put(
                    shard + ':' + addressOrContractID + '_STORAGE_' + storageID,
                    BLOCKCHAIN_GENESIS.STATE[addressOrContractID][storageID]
                )
            }
        } else {
            ...
            atomicBatch.put(
                shardID + ':' + addressOrContractID,
                BLOCKCHAIN_GENESIS.STATE[addressOrContractID]
            ) // else - it's default account
        }
    })
    ...
    */
    ...
    WORKING_THREADS.VERIFICATION_THREAD.WORKFLOW_OPTIONS = {
        ...BLOCKCHAIN_GENESIS.WORKFLOW_OPTIONS
    }
    ...
    vtEpochHandler.quorum = GET_CURRENT_EPOCH_QUORUM(
        vtEpochHandler.poolsRegistry,
        WORKING_THREADS.VERIFICATION_THREAD.WORKFLOW_OPTIONS,
        nullHash
    )
    ...
    qtEpochHandler.quorum = GET_CURRENT_EPOCH_QUORUM(
        qtEpochHandler.poolsRegistry,
        WORKING_THREADS.APPROVEMENT_THREAD.WORKFLOW_OPTIONS,
        nullHash
    )
...
export let PREPARE_BLOCKCHAIN = async () => {
    ...
    let storedApprovementThreadFromDB = await BLOCKCHAIN_DATABASES.APPROVEMENT_THREAD_METADATA.get(
        'AT'
    ).catch(() => null)
    ...
    if (WORKING_THREADS.VERIFICATION_THREAD.VERSION === undefined) {
        ...
        await BLOCKCHAIN_DATABASES.APPROVEMENT_THREAD_METADATA.put(
            'AT',
            WORKING_THREADS.APPROVEMENT_THREAD
        )
    }
    ...
    if (
        ...
        WORKING_THREADS.GENERATION_THREAD.majority = GET_QUORUM_MAJORITY(
            WORKING_THREADS.APPROVEMENT_THREAD.EPOCH
        )
    }
    ...
    let quorumTemporaryDB = level(process.env.CHAINDATA_PATH + `/${epochFullID}`, {
        valueEncoding: 'json'
    })
    ...
    await DECRYPT_KEYS()
        .then(() => LOG(`Private key was decrypted successfully`, COLORS.GREEN))
        .catch(error => {
            LOG(
                `Keys decryption failed.Please,check your password carefully.In the worst case-use your decrypted keys from safezone and repeat procedure of encryption via CLI\n${error}`,
                COLORS.RED
            )
