...
import { COLORS, GET_UTC_TIMESTAMP, LOG } from '../../KLY_Utils/utils.js'
...
export let GET_ALL_KNOWN_PEERS = () => [
    ...CONFIGURATION.NODE_LEVEL.BOOTSTRAP_NODES,
    ...NODE_METADATA.PEERS
]
...
export let EPOCH_STILL_FRESH = thread =>
    thread.EPOCH.startTimestamp + thread.WORKFLOW_OPTIONS.EPOCH_TIME > GET_UTC_TIMESTAMP()
...
export let DECRYPT_KEYS = async () => {
    ...
    LOG(
        `Blockchain info \x1b[32;1m(\x1b[36;1mworkflow:${BLOCKCHAIN_GENESIS.WORKFLOW}[QT major version:${NODE_METADATA.VERSION}] / your pubkey:${CONFIGURATION.NODE_LEVEL.PUBLIC_KEY}\x1b[32;1m)`,
        COLORS.CYAN
    )

    let hexSeed = await new Promise(resolve =>
        readLineInterface.question(
            `\n ${COLORS.TIME_COLOR}[${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}]\u001b[38;5;99m(pid:${process.pid})${COLORS.CLEAR}  Enter \x1b[32mpassword\x1b[0m to decrypt private key in memory of process ———> \x1b[31m`,
            resolve
        )
    )
    ...
    CONFIGURATION.NODE_LEVEL.PRIVATE_KEY =
        decipher.update(CONFIGURATION.NODE_LEVEL.PRIVATE_KEY, 'hex', 'utf8') +
        decipher.final('utf8')
