...
FASTIFY_SERVER.post(
    '/epoch_edge_operation_to_mempool',
    ...
    async (request, response) => {
        ...
        if (
            typeof epochEdgeOperationWithAgreementProofs.epochEdgeOperation !== 'object' ||
            typeof epochEdgeOperationWithAgreementProofs.aggreementProofs !== 'object'
        ) {
            response.send({
                err: `Wrong format. Input data must contain <epochEdgeOperation>(your operation) and <agreementProofs>(aggregated version of verification proofs from quorum members majority)`
            })
        ...
        for (let [signerPubKey, signa] of Object.entries(
            epochEdgeOperationWithAgreementProofs.aggreementProofs
        )) {
            promises.push(
                ED25519_VERIFY(hashOfEpochFullIDAndOperation, signa, signerPubKey).then(
                    isOK => isOK && epochHandler.quorum.includes(signerPubKey) && okSignatures++
                )
            )
        }
        ...
        if (okSignatures >= majority) {
            ...
            currentEpochMetadata.EPOCH_EDGE_OPERATIONS_MEMPOOL.push(
                epochEdgeOperationWithAgreementProofs.epochEdgeOperation
            )
...
FASTIFY_SERVER.post(
    ...
    async (request, response) => {
        ...
        if (EPOCH_EDGE_OPERATIONS_VERIFIERS[epochEdgeOperation.type]) {
            let possibleEpochEdgeOperation = await EPOCH_EDGE_OPERATIONS_VERIFIERS[
                epochEdgeOperation.type
            ](epochEdgeOperation.payload, true, false).catch(error => ({ isError: true, error })) // it's just verify without state changes

            if (possibleEpochEdgeOperation?.isError) {
                response.send({
                    err: `Verification failed. Reason => ${JSON.stringify(possibleEpochEdgeOperation)}`
                })
                ...
                response.send({
                    err: `Verification failed.Check your input data carefully. The returned object from function => ${JSON.stringify(possibleEpochEdgeOperation)}`
                })
        } else
            response.send({
                err: `No verification function for this system sync operation => ${epochEdgeOperation.type}`
            })
    }
)
