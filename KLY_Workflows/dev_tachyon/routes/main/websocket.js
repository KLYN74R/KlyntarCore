import {
    VERIFY_AGGREGATED_EPOCH_FINALIZATION_PROOF,
...
import { COLORS, ED25519_SIGN_DATA, ED25519_VERIFY, LOG } from '../../../../KLY_Utils/utils.js'
...
let RETURN_FINALIZATION_PROOF_FOR_BLOCK = async (parsedData, connection) => {
    ...
    let overviewIsOk =
        typeof block === 'object' &&
        typeof previousBlockAFP === 'object' &&
        !currentEpochMetadata.SYNCHRONIZER.has('STOP_PROOFS_GENERATION:' + block.creator)

    if (
        !CONFIGURATION.NODE_LEVEL.ROUTE_TRIGGERS.MAIN
            .ACCEPT_BLOCKS_AND_RETURN_FINALIZATION_PROOFS ||
        !overviewIsOk
    ) {
        connection.close()
        ...
        !currentEpochMetadata.SYNCHRONIZER.has('GENERATE_FINALIZATION_PROOFS:' + block.creator)
    ) {
        ...
        if (poolIsReal) {
            if (itsPrimePool) {
                ...
                if (currentEpochMetadata.SHARDS_LEADERS_HANDLERS.get(block.creator)) {
                    ...
                    currentEpochMetadata.SYNCHRONIZER.delete(
                        'GENERATE_FINALIZATION_PROOFS:' + block.creator
                    )

                    return
                }
            } else if (
                typeof currentEpochMetadata.SHARDS_LEADERS_HANDLERS.get(block.creator) === 'string'
            ) {
        ...
        let thisLeaderCanGenerateBlocksNow =
            poolIsReal && (itIsReservePoolWhichIsLeaderNow || itsPrimePool)
        ...
        let finalizationStatsForThisPool = currentEpochMetadata.FINALIZATION_STATS.get(
            block.creator
        ) || {
            index: -1,
        ...
        let sameSegment =
            finalizationStatsForThisPool.index < block.index ||
            (finalizationStatsForThisPool.index === block.index &&
                proposedBlockHash === finalizationStatsForThisPool.hash &&
                block.epoch === epochFullID)
        ...
        if (sameSegment) {
            ...
            if (
                await ED25519_VERIFY(proposedBlockHash, block.sig, block.creator).catch(() => false)
            ) {
                ...
                if (block.index === 0) {
                    ...
                    */
                    ...
                    let legacyEpochData = await BLOCKCHAIN_DATABASES.EPOCH_DATA.get(
                        `LEGACY_DATA:${epochHandler.id - 1}`
                    ).catch(() => null) // {epochFullID,quorum,majority}

                    let aefpIsOk =
                        epochHandler.id === 0 ||
                        (legacyEpochData &&
                            (await VERIFY_AGGREGATED_EPOCH_FINALIZATION_PROOF(
                            ...
                            block.extraData.aefpForPreviousEpoch.shard === primePoolPubKey)
                    ...
                    let positionOfBlockCreatorInLeadersSequence = leadersSequence.indexOf(
                        block.creator
                    )

                    let alrpChainIsOk =
                        itsPrimePool ||
                        (await CHECK_ALRP_CHAIN_VALIDITY(
                            primePoolPubKey,
                    ...
                    previousBlockID =
                        epochHandler.id + ':' + block.creator + ':' + (block.index - 1)

                    let itsAfpForPreviousBlock = blockID === previousBlockID

                    if (
                        !itsAfpForPreviousBlock ||
                        typeof prevBlockHash !== 'string' ||
                    ...
                    let isOK = await VERIFY_AGGREGATED_FINALIZATION_PROOF(
                        previousBlockAFP,
                        epochHandler
                    )
                ...
                USE_TEMPORARY_DB(
                    ...
                    .then(() =>
                        ...
                        BLOCKCHAIN_DATABASES.BLOCKS.put(proposedBlockID, block).then(() => {
                            ...
                            BLOCKCHAIN_DATABASES.EPOCH_DATA.put('AFP:' + previousBlockID, {
                                prevBlockHash,
                                blockID,
                                blockHash,
                                proofs
                            }).then(async () => {
                                currentEpochMetadata.FINALIZATION_STATS.set(
                                    block.creator,
                                    futureMetadataToStore
                                )

                                let dataToSign =
                                    (previousBlockAFP.blockHash ||
                                        '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef') +
                                    proposedBlockID +
                                    proposedBlockHash +
                                    epochFullID

                                let finalizationProof = await ED25519_SIGN_DATA(
                                    dataToSign,
                                ...
                                let tmbProof = await ED25519_SIGN_DATA(
                                    dataToSign,
                                    CONFIGURATION.NODE_LEVEL.PRIVATE_KEY
                                )
...
let RETURN_FINALIZATION_PROOF_BASED_ON_TMB_PROOF = async (parsedData, connection) => {
    ...
    let { blockCreator, blockIndex, blockHash, previousBlockAFP, tmbProofs, tmbTicketID } =
        parsedData

    let typeCheckOverviewIsOk =
        typeof blockCreator === 'string' &&
        typeof blockHash === 'string' &&
        typeof blockIndex === 'number' &&
        typeof tmbTicketID === 'number' &&
        typeof previousBlockAFP === 'object' &&
        typeof tmbProofs === 'object' &&
        !currentEpochMetadata.SYNCHRONIZER.has('STOP_PROOFS_GENERATION:' + blockCreator)

    if (
        !CONFIGURATION.NODE_LEVEL.ROUTE_TRIGGERS.MAIN
            .ACCEPT_BLOCKS_AND_RETURN_FINALIZATION_PROOFS ||
        !typeCheckOverviewIsOk
    ) {
        connection.close()

        return
    } else if (
        !currentEpochMetadata.SYNCHRONIZER.has('GENERATE_FINALIZATION_PROOFS:' + blockCreator)
    ) {
        // Add the sync flag to prevent creation proofs during the process of skip this pool
        ...
        let sameSegment =
            finalizationStatsForThisPool.index < blockIndex ||
            (finalizationStatsForThisPool.index === blockIndex &&
                proposedBlockHash === finalizationStatsForThisPool.hash)
        ...
        if (sameSegment) {
            ...
            let dataToSignToApproveProposedBlock =
                (previousBlockAFP.blockHash ||
                    '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef') +
                proposedBlockID +
                proposedBlockHash +
            ...
            let subsetOfValidators = GET_PSEUDO_RANDOM_SUBSET_FROM_QUORUM_BY_TICKET_ID(
                tmbTicketID,
                epochHandler
            )

            let dataThatShouldBeSignedInTMB =
                dataToSignToApproveProposedBlock + 'VALID_BLOCK_RECEIVED'
            ...
            for (let choosenValidator of subsetOfValidators) {
                let signaIsOk = await ED25519_VERIFY(
                    dataThatShouldBeSignedInTMB,
                    tmbProofs[choosenValidator],
                    choosenValidator
                )
            ...
            if (tmbVerificationIsOk) {
                ...
                let {
                    prevBlockHash,
                    blockID: blockIDFromAFP,
                    blockHash: blockHashFromAFP,
                    proofs
                } = previousBlockAFP

                if (blockIndex !== 0) {
                    let previousBlockID =
                        epochHandler.id + ':' + blockCreator + ':' + (blockIndex - 1)

                    let itsReallyAfpForPreviousBlock = blockIDFromAFP === previousBlockID

                    if (
                        !itsReallyAfpForPreviousBlock ||
                        typeof prevBlockHash !== 'string' ||
                        typeof blockIDFromAFP !== 'string' ||
                        typeof blockHashFromAFP !== 'string' ||
                        typeof proofs !== 'object'
                    ) {
                        connection.close()

                        currentEpochMetadata.SYNCHRONIZER.delete(
                            'GENERATE_FINALIZATION_PROOFS:' + blockCreator
                        )
                ...
                USE_TEMPORARY_DB(
                    ...
                    .then(() => {
                        ...
                        BLOCKCHAIN_DATABASES.EPOCH_DATA.put('AFP:' + blockIDFromAFP, {
                            prevBlockHash,
                            blockID: blockIDFromAFP,
                            blockHash: blockHashFromAFP,
                            proofs
                            ...
                            let finalizationProof = await ED25519_SIGN_DATA(
                                dataToSignToApproveProposedBlock,
                                CONFIGURATION.NODE_LEVEL.PRIVATE_KEY
                            )
...
let RETURN_BLOCKS_RANGE = async (data, connection) => {
    ...
    for (let i = 1; i < 50; i++) {
        let blockIdToFind =
            data.epochIndex +
            ':' +
            CONFIGURATION.NODE_LEVEL.PUBLIC_KEY +
            ':' +
            (data.hasUntilHeight + i)

        let blockIdToFindAfp =
            data.epochIndex +
            ':' +
            CONFIGURATION.NODE_LEVEL.PUBLIC_KEY +
            ':' +
            (data.hasUntilHeight + i + 1)

        let block = await BLOCKCHAIN_DATABASES.BLOCKS.get(blockIdToFind).catch(() => null)

        let afpForBlock = await BLOCKCHAIN_DATABASES.EPOCH_DATA.get(
            'AFP:' + blockIdToFindAfp
        ).catch(() => null)
...
server.listen(
    CONFIGURATION.NODE_LEVEL.WEBSOCKET_PORT,
    CONFIGURATION.NODE_LEVEL.WEBSOCKET_INTERFACE,
    () =>
        LOG(
            `Websocket server was activated on \u001b[38;5;168m${CONFIGURATION.NODE_LEVEL.WEBSOCKET_INTERFACE}:${CONFIGURATION.NODE_LEVEL.WEBSOCKET_PORT}`,
            COLORS.CD
        )
)
    ...
    connection.on('message', message => {
        if (message.type === 'utf8') {
            ...
            if (data.route === 'get_finalization_proof') {
                ...
                connection.close(
                    1337,
                    'No available route. You can use <get_commitment_for_block_range> | <get_finalization_proof_for_range>'
                )
            }
        }
    })
