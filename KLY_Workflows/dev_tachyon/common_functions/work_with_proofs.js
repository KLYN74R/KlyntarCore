...
export let VERIFY_AGGREGATED_EPOCH_FINALIZATION_PROOF = async (
    ...
    let overviewIsOK =
        ...
        typeof itsProbablyAggregatedEpochFinalizationProof.hashOfFirstBlockByLastLeader ===
            'string' &&
    ...
    if (overviewIsOK && itsProbablyAggregatedEpochFinalizationProof) {
        ...
        */
        ...
        let { shard, lastLeader, lastIndex, lastHash, hashOfFirstBlockByLastLeader } =
            itsProbablyAggregatedEpochFinalizationProof
        ...
        for (let [signerPubKey, signa] of Object.entries(
...
export let VERIFY_AGGREGATED_FINALIZATION_PROOF = async (
    itsProbablyAggregatedFinalizationProof,
    epochHandler
) => {
    // Make the initial overview
    let generalAndTypeCheck =
        itsProbablyAggregatedFinalizationProof &&
...
export let GET_VERIFIED_AGGREGATED_FINALIZATION_PROOF_BY_BLOCK_ID = async (
    blockID,
    epochHandler
) => {
    let localVersionOfAfp = await BLOCKCHAIN_DATABASES.EPOCH_DATA.get('AFP:' + blockID).catch(
        () => null
    )
    ...
    if (!localVersionOfAfp) {
        ...
        for (let endpoint of setOfUrls) {
            let itsProbablyAggregatedFinalizationProof = await fetch(
                endpoint + '/aggregated_finalization_proof/' + blockID
            )
                .then(r => r.json())
                .catch(() => null)
            ...
            if (itsProbablyAggregatedFinalizationProof) {
                ...
                if (isOK) {
                    let { prevBlockHash, blockID, blockHash, proofs } =
                        itsProbablyAggregatedFinalizationProof
...
export let GET_FIRST_BLOCK_ON_EPOCH = async (epochHandler, shardID, getBlockFunction) => {
    ...
    if (!pivotPoolData) {
        ...
        for (
            let position = -1, length = arrayOfReservePoolsForShard.length;
            position < length;
            position++
        ) {
            ...
            let afp = await GET_VERIFIED_AGGREGATED_FINALIZATION_PROOF_BY_BLOCK_ID(
                firstBlockIDByThisPubKey,
                epochHandler
            )

            let potentialFirstBlock = await getBlockFunction(
                epochHandler.id,
                potentialPivotPubKey,
                0
            )

            if (
                afp &&
                afp.blockID === firstBlockIDByThisPubKey &&
                potentialFirstBlock &&
                afp.blockHash === Block.genHash(potentialFirstBlock)
            ) {
    ...
    if (pivotPoolData) {
        ...
        if (pivotPoolData.position === -1) {
            ...
            for (let position = pivotPoolData.position - 1; position >= -1; position--) {
                ...
                let leaderRotationProofForPreviousPool =
                    blockToEnumerateAlrp.extraData.aggregatedLeadersRotationProofs[
                        previousPoolInLeadersSequence
                    ]
                ...
                if (previousPoolInLeadersSequence === shardID) {
                    ...
                    if (leaderRotationProofForPreviousPool.skipIndex === -1) {
                        ...
                        return {
                            firstBlockCreator: pivotPoolData.pivotPubKey,
                            firstBlockHash: pivotPoolData.firstBlockHash
                        }
                    } else {
                    ...
                    let firstBlockByNewPivot = await getBlockFunction(
                        epochHandler.id,
                        previousPoolInLeadersSequence,
                        0
                    )
                    ...
                    if (
                        firstBlockByNewPivot &&
                        leaderRotationProofForPreviousPool.firstBlockHash ===
                            Block.genHash(firstBlockByNewPivot)
                    ) {
