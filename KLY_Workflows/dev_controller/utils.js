...
*/
export let GET_ACCOUNT_ON_SYMBIOTE = addr =>
...
DECRYPT_KEYS=async(spinner,role)=>{
    ...
    if(CONFIG.PRELUDE.DECRYPTED){
        ...
        Object.keys(CONFIG.SYMBIOTE.MANIFEST.HOSTCHAINS).forEach(
            ...
            ticker => {
                ...
                if(CONFIG.EVM_CHAINS.includes(ticker)) HOSTCHAINS.CONNECTORS.get(ticker).PRV=Buffer.from(keys[ticker],'hex')
    ...
    Object.keys(symbioteConfigurationReference.CONNECTORS).forEach(ticker=>{
        ...
        if(CONFIG.EVM_CHAINS.includes(ticker)) HOSTCHAINS.CONNECTORS.get(ticker).PRV=Buffer.from(privateKey,'hex')
