import { BLAKE3, SIG, VERIFY } from './crypto_utils.js'
import l from 'level'
import fs from 'fs'
...
let db = l('TEST', { valueEncoding: 'json' }),
    ...
    instantBlock = {
        ...
        d: [],
        s: [],
        n: 'RqtrnrLAdxpUkjqKS42RKbgN1ryXad3NeJrPTBZpdyVL',
        sig: ''
    }
...
await new Promise((resolve, _reject) =>
    db
        .createKeyStream()
        .on('data', address => defaultTxs.push({ c: address, d: 'ANOTHER ONE', n: 19 }))
        .on('end', () => {
            instantBlock.d = defaultTxs
            // console.log('TXS SIZE IS -> ',instantBlock.d.length)
            // console.log('TXS -> ',instantBlock.d)
            db.createKeyStream()
                .on('data', address => defaultTxs.push({ c: address, d: 'FIN7', n: 18 }))
                .on('end', () => {
                    resolve()
                })
        })
...
instantBlock.sig = await SIG(
    hash,
    'MC4CAQAwBQYDK2VwBCIEIO92P6igIVKRplDK7LT1k1fUlOgN8kcKH2Osb/dCqv2y'
)
...
let controllerBlock = {
    ...
    GET_CHAIN_ACC = addr =>
        ...
        db
            .get(addr)
            .then(ACCOUNT =>
                ACCOUNTS.set(addr, { ACCOUNT, NS: new Set(), ND: new Set(), OUT: ACCOUNT.B }).get(
                    addr
                )
            )
            .catch(e => false),
    controllerHash = BLAKE3(
        JSON.stringify(controllerBlock.a) + CHAIN + controllerBlock.i + controllerBlock.p
    )
        ...
        if (await VERIFY(controllerHash, controllerBlock.sig, chain)) {
                ...
                ['d', 's'].forEach(type =>
                    txsSet[type].forEach(tx => {
                        ...
                        if (!BLACKLIST.has(tx.c)) {
                            let acc = GET_CHAIN_ACC(tx.c),
                                spend = 1 + (tx.a || (tx.m && 10000) || 0)

                            tx.n <= acc.ACCOUNT.N || acc.NS.has(tx.n)
                                ? acc.ND.add(tx.n)
                                : acc.NS.add(tx.n)
            ...
            txsToSift.forEach(txsSet => {
                txsSet.d.forEach(obj => {
                    if (obj.a)
                        txsPromises.push(vTX2(obj.c, obj.r, obj.a, rewardBox.get(hash), obj.n))
                    else if (obj.h)
                        txsPromises.push(vNTX2(obj.c, obj.h, rewardBox.get(hash), obj.n))
                    else if (obj.d)
                        txsPromises.push(vDLG2(obj.c, obj.d, rewardBox.get(hash), obj.n))
                    ...
                    if (obj.a)
                        txsPromises.push(
                            vTX2(
                                ...
                                obj.t,
                                obj.a,
                                'IzQ3kPHHW7IEjBl7UMVcdpXRoyfekGqadsBxBnLQ3Ss=',
                                chain,
                                obj.n
                            )
                        )
                        ...
                        txsPromises.push(vNTX2(obj.c, obj.h, instantAddress, chain, obj.n))
                    else if (obj.d)
                        txsPromises.push(
                            vDLG2(
                                obj.c,
                        ...
                        // TODO fix the following line
                        // eslint-disable-next-line
                        txsPromises.push(vCR2(obj.c, obj.m, instantAddress, chain, obj.n))
                })
            })
            ...
            ACCOUNTS.forEach((acc, addr) => promises.push(db.put(addr, acc.ACCOUNT)))
