...
const NUMBER_OF_VALIDATORS = 1000
...
if (THIRD_PART_OF_QUORUM > NUMBER_OF_BAD_ACTORS) {
    console.log(
        'Total number of bad actors in general is less than third party of quorum, so impossible to take control over network'
    )
} else {
    for (
        let numberOfBadActorsInCurrentQuorum = BigInt(THIRD_PART_OF_QUORUM);
        numberOfBadActorsInCurrentQuorum <= BigInt(limit);
        numberOfBadActorsInCurrentQuorum++
    ) {
        if (numberOfBadActorsInCurrentQuorum > BigInt(NUMBER_OF_BAD_ACTORS)) break

        let casesWhenMoreThanOneThirdBadActors = getKFromN(
            numberOfBadActorsInCurrentQuorum,
            BigInt(NUMBER_OF_BAD_ACTORS)
        )

        let numberOfNormalActorsInThisQuorum =
            BigInt(QUORUM_SIZE) - numberOfBadActorsInCurrentQuorum

        let casesWhenNorm = getKFromN(
            numberOfNormalActorsInThisQuorum,
            BigInt(NUMBER_OF_NORM_ACTORS)
        )

        let probability =
            Number(casesWhenMoreThanOneThirdBadActors * casesWhenNorm) /
            Number(totalNumberOfPossibleCases)

        console.log(
            `Bad actors: ${numberOfBadActorsInCurrentQuorum} | Norm actors: ${numberOfNormalActorsInThisQuorum} | Probability: ${probability}`
        )
