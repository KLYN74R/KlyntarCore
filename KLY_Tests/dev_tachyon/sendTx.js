...
let tblsAccounts = {
    A: {
        ...
        shares: [
            'f77ffec138a41033c3f278774685a53fa4f49687365d788c0efd671f29359703',
            'f5b187d48cd56e9680985f5f0a6f7744bef11092a142bbbc0d5f0c4c92ed0a02',
            '48f13b18936fabb681d7ed21bd0e747e9553b3777547795e9ad878734197631a'
        ],
    ...
    B: {
        ...
        shares: [
            '1bfe78127fb2e4ee5eab8d0875aef9c49606535825fbb2619edb29ccd1a93508',
            'ba706c61ab4be0b619b523021e79afddf21e124ecca08c1af4d057b3b6dd7213',
            'a97319f37cdf2833a996334952eb73f8903260574d1ef549007a0d2de1ccb41c'
        ],
    ...
    C: {
        vv: [
            '0e4b8473bfe882f056d3ce4de2550b2f3415ec83b04fc9580656e74e308fa207730a126afe151feef1b505b3a00bc5939d6b55a622d47fdf3e71baabe614c89a',
        ...
        shares: [
            '4d941e9d6755581106df5189c54cffbc104709867f3c77078f3ac17c0880fc1e',
            'cb00cad4e2c6d490d322ec0eb55ee7e381cd2e6a0e598eca12a1cf06a918791a',
            '315dd8656357528f41e6140cf01b9f40cd01d1e2b4fafad40e706bc08aa45912'
        ],
    ...
    rootPub:
        'bedc88644f0deea4c0a77ba687712f494a1af7d8869f09768a0db42284f89d17b7b9225e0c87c2cb5511907dfd5eae3a53d789298721039e833770de29595880'
}
...
let GET_TX_TEMPLATE = async (account, txType, sigType, nonce, payload) => {
    ...
    if (sigType === SIG_TYPES.DEFAULT) {
        template.sig = await ED25519_SIGN_DATA(
            SYMBIOTE_ID + WORKFLOW_VERSION + txType + JSON.stringify(payload) + nonce + FEE,
            account.prv
        )
    } else if (sigType === SIG_TYPES.MULTISIG) {
        template.sig = await bls.singleSig(
            SYMBIOTE_ID + WORKFLOW_VERSION + txType + JSON.stringify(payload) + nonce + FEE,
            account.prv
        )
    }
...
let SEND_TRANSACTION = transaction => {
    return fetch(
        'http://localhost:7331/transaction',
...
let MULTISIG_2_DEFAULT = async () => {
    ...
    let transaction = await GET_TX_TEMPLATE(
        user0,
        TX_TYPES.TX,
        SIG_TYPES.MULTISIG,
        accData.nonce + 1,
...
let MULTISIG_2_MULTISIG = async () => {
    ...
    let msig2MsigPayload = {
        ...
        type: 'M',
        rev_t: 0,
        to: '6TSGRz9KaTHtwtFXdLHoyvn1F5uQEysqz43nMH5DY3Zh2xtmKeuZST5PZR1zZVsCHk',
        amount: 1000
    }
...
let TBLS_2_DEFAULT = async () => {
    ...
    let dataToSign =
        ...
        WORKFLOW_VERSION +
        template.type +
        JSON.stringify(tblsPayload) +
        template.nonce +
        FEE

    //_____________ GENERATE SIG_SHARES _____________

    let shareSigA = JSON.parse(
        tbls.signTBLS(
            tblsAccounts.A.id,
            [
                { secretKeyShare: tblsAccounts.A.shares[0] },
                { secretKeyShare: tblsAccounts.B.shares[0] },
                { secretKeyShare: tblsAccounts.C.shares[0] }
            ],
            dataToSign
        )
    )

    let shareSigB = JSON.parse(
        tbls.signTBLS(
            tblsAccounts.B.id,
            [
                { secretKeyShare: tblsAccounts.A.shares[1] },
                { secretKeyShare: tblsAccounts.B.shares[1] },
                { secretKeyShare: tblsAccounts.C.shares[1] }
            ],
            dataToSign
        )
    )

    let shareSigC = JSON.parse(
        tbls.signTBLS(
            tblsAccounts.C.id,
            [
                { secretKeyShare: tblsAccounts.A.shares[2] },
                { secretKeyShare: tblsAccounts.B.shares[2] },
                { secretKeyShare: tblsAccounts.C.shares[2] }
            ],
            dataToSign
        )
    )
